...
expr: INTEGER_NUMBER                                                                                    { $$=create_const_integer_expr(Integer, $1); }
    ...
    | SUPER                                                                                             { puts("SUPER are not allowed\n");}
    ...
    | NOT new_lines_op expr                                                                             { $$=create_op_expr(not_keyword, $3, 0); }
    | expr AND new_lines_op expr                                                                        { $$=create_op_expr(and_keyword, $1, $4); }
    | expr OR new_lines_op expr                                                                         { $$=create_op_expr(or_keyword, $1, $4); }
    ...
    | SUPER OPEN_ROUND_BRACKET new_lines_op expr_list CLOSE_ROUND_BRACKET                               { puts("SUPER are not allowed\n"); }
...
class_declaration: CLASS VAR_OR_METHOD_NAME stmt_ends def_method_list_op END stmt_ends                      { $$=create_class_declaration_program_item($2, 0, $4); }
    | CLASS VAR_OR_METHOD_NAME LESS_OP new_lines_op VAR_OR_METHOD_NAME stmt_ends def_method_list_op END stmt_ends   { $$=create_class_declaration_program_item($2, $5, $7);  }
    ;
